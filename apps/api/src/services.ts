...
export const minio = new Client({endPoint: process.env.MINIO_ENDPOINT!, port: Number(process.env.MINIO_PORT), useSSL: false, accessKey: process.env.MINIO_ACCESS_KEY!, secretKey: process.env.MINIO_SECRET_KEY!});
// This client only signs browser-facing URLs. The API itself always uses the internal Docker hostname above.
export const minioPublic = new Client({endPoint: process.env.MINIO_PUBLIC_ENDPOINT!, port: Number(process.env.MINIO_PORT), useSSL: false, accessKey: process.env.MINIO_ACCESS_KEY!, secretKey: process.env.MINIO_SECRET_KEY!});
