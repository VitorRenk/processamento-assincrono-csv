import amqp from 'amqplib'; import {PrismaClient} from '@prisma/client'; import Redis from 'ioredis'; import {Client} from 'minio'; import {analyseCsvStream} from '@fluxocsv/shared/csv'; import {reportCsv, reportHtml} from '@fluxocsv/shared/report'; import {DEAD_LETTER_QUEUE, PROCESSING_QUEUE, processingQueueOptions} from '@fluxocsv/shared/queue';
const prisma = new PrismaClient(); const redis = new Redis(process.env.REDIS_URL!); const bucket = 'fluxocsv'; const queue = PROCESSING_QUEUE; const reconnectDelayMs = 3000;
...
function scheduleReconnect(reason: unknown) {
  ...
  console.error('Worker desconectado; nova tentativa em 3 segundos.', reason);
  reconnectTimer = setTimeout(() => { reconnectTimer = undefined; void boot(); }, reconnectDelayMs);
}
async function boot() {
  try {
    const connection = await amqp.connect(process.env.RABBITMQ_URL!);
    connection.on('error', error => console.error('Erro na conexão RabbitMQ.', error));
    connection.on('close', () => scheduleReconnect('Conexão RabbitMQ fechada.'));
    const channel = await connection.createChannel();
    channel.on('error', error => console.error('Erro no canal RabbitMQ.', error));
    await channel.assertQueue(queue, processingQueueOptions);
    ...
    await channel.consume(queue, async (message) => {
      ...
      try {
        ...
        if ((job?.attempts ?? 0) < 3) {
          ...
          channel.sendToQueue(DEAD_LETTER_QUEUE, Buffer.from(JSON.stringify({jobId, error: text})), {persistent: true});
        }
        channel.ack(message);
      }
    });
    console.log('Worker conectado e aguardando jobs.');
